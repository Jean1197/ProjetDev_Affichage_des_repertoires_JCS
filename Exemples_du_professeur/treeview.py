...
def populate_tree(tree, parent, folder):
    ...
    for item in folder.iterdir():
        item_name = f"📁 {item.name}" if item.is_dir() else f"📄︎{item.name}"
...
file_menu.add_command(label="Hello", command=lambda : print("hello"))
...
style.configure(
    ...
    font=("Arial", 12),
    rowheight=28,
    background="lightblue", #couleur du fond
    #fieldbackground="lightgreen",
    foreground="blue" #couleur d'écriture
)
